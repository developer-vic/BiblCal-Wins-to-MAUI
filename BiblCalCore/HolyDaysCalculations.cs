...
{
    /// <summary>
    /// Holy Days calculations
    /// Direct port from Windows modBiblcalFunctions.CalcHolyDays
    /// Matches Windows app calculations exactly
    /// </summary>
    public class HolyDaysCalculations
    {
        ...
        private const double VEQ = 6.79189999999999E-02; // Constant for Spring Equinox calculation
        private const double JDNJLD = 1170111.5; // Julian Day number of Jeshua's long day
        private const double JDNJLDBench = 1170111.5; // Benchmark
        ...
        private static readonly string[] DayName = { "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" };
        ...
        // Variables for Holy Days calculation
        private double _aa = 0; // Julian Day count for holy days
        private double _d = 0; // Day of week
        private double _w = 0;
        private double _lo = 0; // Sun's true geometric Longitude
        private double _hs = 0; // Hour of sunset
        private double _hr = 0; // Hour location (Jerusalem)
        private double _lt = 0; // Latitude (Jerusalem) - in degrees
        private double _lg = 0; // Longitude (Jerusalem) - in degrees
        private double _vs = 0; // Visibility criteria
        private bool _sunsetFlag = false; // Tells that 'Sunset' routine is active
        private bool _earlyYear = false;
        private bool _mayBeEarlyOrLate = false;
        private double _monthNumber = 0;
        private double _jdd = 0; // Vernal Equinox
        private double _jdt = 0; // Visual New Moon
        private double _dmt = 0;
        private double _yri = 0;
        private double _yearAfterCreation = 0;
        private double _jubileeYear = 0;
        private double _sabbathYear = 0;
        private double _ly = 0;
        private double _m0 = 0; // Month counter for New Moons calculation
        private string _ejwString = "";

        // Additional calculation variables (matching LocalMoonCalculations)
        ...
        public HolyDaysCalculations(IOutputWriter outputWriter, BiblicalCalendarCalculator calculator)
        {
        ...
        public void CalculateHolyDays(double gregorianYear, string ejwString = "", double? latitude = null, double? longitude = null, double? utcOffset = null, string locationName = "")
        {
            ...
            if (gregorianYear < 0)
            {
                _yearAfterCreation = gregorianYear + 4005; // BCE is 4005
            }
            else
            {
            ...
            // Jubilee calendar calculations
            CalculateJubileeYear(gregorianYear);
        ...
        public void CalculateNewMoons(double gregorianYear, string ejwString = "", double? latitude = null, double? longitude = null, double? utcOffset = null, string locationName = "")
        {
            ...
            if (_ejwString == "Y")
            {
                ...
                _outputWriter.WriteLine("for the region east of Israel and west of the International Date Line.");
                _outputWriter.WriteLine("");
            }
            else
            {
                _outputWriter.WriteLine("These dates are for Israel and regions westward to the International Date Line.");
                _outputWriter.WriteLine("");
            }

            // Calculate starting month number (matching Windows CalcNewMoons lines 513-521)
            _ly = gregorianYear + 0.256;
            if (_ly >= 0)
            {
                _ln = Math.Floor((_ly - 1900) * 12.368277);
            }
            else
            {
                _ln = Math.Floor((_ly - 1899) * 12.368277);
            }

            // Calculate for 12-13 months (matching Windows CalcNewMoons lines 522-548)
            _m0 = 0;
            while (_m0 < 14)
            {
                FindFirstDay();
        ...
        private void InitializeCalculationVariables()
        {
            ...
            // Set up calculator with Jerusalem coordinates
            // First set the calculator's LG, LT, HR from our instance variables
            _calculator.LG = _lg; // In degrees (longitude)
            _calculator.LT = _lt; // In degrees (latitude)
            _calculator.HR = _hr;

            // Now get them back for calculations (matching Windows pattern)
        ...
        private void CalculateJubileeYear(double gregorianYear)
        {
            // Matching Windows Jubilee calendar code (lines 281-325)
            double firstJubileeYear = 2544; // If the cycle started at the Jordan crossing 1510 BCE
            double lastJubileeYear = 10000;

            if (_yearAfterCreation >= (firstJubileeYear - 48) && _yearAfterCreation <= lastJubileeYear)
            {
                _jubileeYear = (_yearAfterCreation - firstJubileeYear) / 50.0;
                _sabbathYear = (Convert.ToInt32((_yearAfterCreation - firstJubileeYear - 49) - Math.Floor(_jubileeYear))) % 7;

                if (_jubileeYear - Math.Floor(_jubileeYear) == 0)
                {
                    string jubileeYearString = FormatNumber(_jubileeYear + 1);
                    _outputWriter.Write($"This may be the{jubileeYearString}");

                    if (_jubileeYear > 10 && _jubileeYear < 20)
                    {
                        _outputWriter.Write("th ");
                    }
                    else
                    {
                        int lastDigit = Convert.ToInt32(double.Parse(jubileeYearString.Trim().Substring(Math.Max(jubileeYearString.Trim().Length - 1, 0))));
                        if (lastDigit == 0)
                        {
                            _outputWriter.Write("th ");
                        }
                        ...
                        {
                            _outputWriter.Write("st ");
                        }
                        else if (lastDigit == 2)
                        {
                            _outputWriter.Write("nd ");
                        }
                        else if (lastDigit == 3)
                        {
                            _outputWriter.Write("rd ");
                        }
                        else if (lastDigit > 3)
                        {
                            _outputWriter.Write("th ");
                    ...
                    _outputWriter.WriteLine("Jubilee (Yobel) Year.");
                }

                if (_sabbathYear == 0 && _jubileeYear - Math.Floor(_jubileeYear) != 0)
                {
                    _outputWriter.WriteLine("This may be a Sabbatical (Shemittah) Year.");
                }
            }
            _outputWriter.WriteLine("");
        }

        private void CalculateYearHolyDays(double gregorianYear)
        {
            // Matching Windows CalculateYearHolyDays (lines 328-484)
            _ly = gregorianYear + 0.256;
            if (_ly >= 0)
            {
                _ln = Math.Floor((_ly - 1900) * 12.368277);
            ...
            do
            {
                ...
                if (loopCount > 100)
                {
                    throw new InvalidOperationException($"Possible infinite loop detected in CalculateYearHolyDays after {loopCount} iterations");
                }
                FindFirstDay();
                
                // Spring/vernal equinox calculation
                _yri = Math.Floor(gregorianYear);
                if (_yri < 0)
                {
                    _yri++;
                }

                _jdd = 1721139.2855 + 365.2421376 * _yri + VEQ * Math.Pow(_yri / 1000.0, 2) - 0.0027879 * Math.Pow(_yri / 1000.0, 3);
                _jdt = (_aa - 1) + (_hs + 0.25) / 24.0 + _hr / 24.0;
                _dmt = Math.Abs(_jdd - _jdt);

                if (_dmt < 1)
                {
                    _jt = (_jdd - 2415020) / 36525.0;
                    if (_jdd < 1483746)
                    {
                        _jt -= 0.00000076;
                        FiftyFourHundred();
                ...
                if (_jdd > _jdt)
                {
                    _mayBeEarlyOrLate = true;
                    _outputWriter.WriteLine("If this Spring's barley harvest is/was early, this New Moon will begin the New Year. ");
                    _earlyYear = true;
                }
                else
                {
                ...
                if (_vs == 1)
                {
                    _outputWriter.WriteLine("Abib 1, Passover sacrifice and Feast of Unleavened Bread are possibly one day earlier.");
                }
                if (_vs == 2)
                {
                    _outputWriter.WriteLine("Abib 1, Passover sacrifice and Feast of Unleavened Bread are possibly one day later.");
                }

                _outputWriter.Write("Abib 1 is ");
                PrintDayAndDate();

                _outputWriter.Write("The Passover sacrifice is ");
                _aa += 13;
                PrintDayAndDate();

                _outputWriter.Write("The Feast of Unleavened Bread begins on ");
                _aa++;
                PrintDayAndDate();
                _outputWriter.Write(" and ends on ");
                _aa += 6;
                PrintDayAndDate();

                // Wave Offering and Pentecost
                if (_ejwString == "Y")
                {
                    _outputWriter.Write("The Wave Offering (the First-fruit) is Monday, ");
                    _d--;
                    if (_d < 0)
                    {
                        _d = 6;
                    }
                    _d = Convert.ToInt32(_d);
                    _w = 7 - _d;
                    _aa = _aa + _w - 6;
                    PrintDate();
                    
                    _outputWriter.Write("First-fruits (Pentecost) is Monday, ");
                    _aa += 50;
                    PrintDate();

                    if (_d == 6 && _vs == 2)
                    {
                        _outputWriter.WriteLine("If Abib 1 is later then Wave Offering and Pentecost are one week later.");
                    }
                    if (_d == 0 && _vs == 1)
                    {
                        _outputWriter.WriteLine("If Abib 1 is earlier then Wave Offering and Pentecost are one week earlier.");
                    }
                }
                else
                {
                    _outputWriter.Write("The Wave Offering (the First-fruit) is Sunday, ");
                    _w = 7 - _d;
                    _aa = _aa + _w - 7;
                    PrintDate();
                    _outputWriter.WriteLine("");

                    _outputWriter.Write("First-fruits (Pentecost) is Sunday, ");
                    _aa += 49;
                    PrintDate();
                    _outputWriter.WriteLine("");
                ...
                if (_vs == 1)
                {
                    _outputWriter.WriteLine("The following Holy Days could be one day earlier:");
                }
                if (_vs == 2)
                {
                    _outputWriter.WriteLine("The following Holy Days could be one day later:");
                }

                _outputWriter.Write("The Day of Trumpets is ");
                PrintDayAndDate();

                _outputWriter.Write("The Day of Atonement is ");
                _aa += 9;
                PrintDayAndDate();

                _outputWriter.Write("The Feast of Tabernacles (Ingathering) begins on ");
                _aa += 5;
                PrintDayAndDate();
                _outputWriter.Write(" and ends on ");
                _aa += 6;
                ...
                _outputWriter.Write("The Last Great Day is ");
                _aa++;
                PrintDayAndDate();
                ...
                {
                    _outputWriter.WriteLine("");
                    _outputWriter.WriteLine("If this Spring's barley harvest is/was late, God's Holy Days are:");
                    _ln -= 5;
                }
                else
                {
        ...
        private void FindFirstDay()
        {
            ...
            // Print table header (lines 556-557)
            ...
            // Convert to Gregorian date for calculations
            FiftyTwoFifty();

            // Set AA to the Julian Day count for the first day
            _aa = _ji + 1;
        }
        ...
        private void FiftyTwoFifty()
        {
            ...
            _daye = b - g - Math.Floor(30.6001 * e) + _jf;
            ...
            if (_gyear < 1)
            {
                _gyear--;
                    ...
                    // Print date (matching Windows FiftyTwoFifty lines 620-626)
                    string dayString = Math.Floor(_daye).ToString(CultureInfo.InvariantCulture);
                    dayString = dayString.TrimStart(' ', '-');
                    while (dayString.Length < 2)
                    {
                        dayString = " " + dayString;
                    }
                    _outputWriter.Write(dayString + " " + MonthName[(int)_m]);

                    // Calculate sunset time for visibility calculations
                    if (_ji < 1483746)
                    {
                        _jt = (_ji + _calculator.LG + 0.2222 + _dt - 2415020) / 36525.0;
                    }
                    else
                    {
                        _jt = (_ji + _calculator.LG + 0.25 + _dt - 2415020) / 36525.0;
                    }
        ...
        {
            // Matching Windows FiftyFourHundred (lines 639-830+)
            // For Holy Days, we calculate VS and HS but don't write output
            ...
            if (_dmt == 0)
            {
                ...
                {
                    moonLoopCount++;
                    if (moonLoopCount > 1000)
                    {
                        throw new InvalidOperationException($"Possible infinite loop in FiftyFourHundred moon position calculation after {moonLoopCount} iterations");
                    }
                    FindPositionOfMoon();
                    if (_mc != 0)
                    {
                        moonsetFound = true;
                        _hoString = FormatNumber(_ho);
                        _hqString = FormatNumber(_hq);
                        _hqString = _hqString.TrimStart();
                    }
                    else
                    {
                        moonsetFound = false;
                ...
                // Print moonset time (matching Windows FiftyFourHundred lines 703-710)
                if (_hq < 10)
                {
                ...
                // Print illumination (matching Windows FiftyFourHundred line 718)
                ...
                // Print Sun's Azimuth (matching Windows FiftyFourHundred line 719)
                _outputWriter.Write("  " + _azsString);
                // Print Moon's Azimuth (matching Windows FiftyFourHundred line 748)
                _outputWriter.Write("  " + _azString);
                // Print Moon's Altitude (matching Windows FiftyFourHundred line 750)
                _outputWriter.Write("   " + _halString);
                NauticalTwilightCalc();
                // Print Sun's Altitude (matching Windows FiftyFourHundred line 1027)
                ...
                // Print Visibility Number (matching Windows FiftyFourHundred line 755)
                ...
                if (_visibilityNumber > 88 && _visibilityNumber <= 100)
                {
                    ...
                    _aa = _ji + 2;
                    _mc = 0;
                    // Print visibility status (matching Windows FiftyFourHundred line 773)
                    _outputWriter.WriteLine("  Prob Not Visible");
                    // For Holy Days mode, find total visible day and probability
                    _ji++;
                    FiftyTwoFifty();
                    _vs = 1;
                    _aa = _ji + 1;
                ...
                else
                {
                    if (_visibilityNumber > 100 && _visibilityNumber <= 112)
                    {
                        ...
                        _aa = _ji + 1;
                        _mc = 0;
                        // Print visibility status (matching Windows FiftyFourHundred line 798)
                    ...
                    else
                    {
                        ...
                        _aa = _ji + 1;
                        _mc = 0;
                        // Print visibility status (matching Windows FiftyFourHundred line 817)
        ...
        private void PrintSunset()
        {
            ...
            // For Holy Days, we calculate HS but don't write output
            ...
            if (_dmt == 0)
            {
                ...
                // Print sunset time (matching Windows PrintSunset lines 1096-1107)
                _outputWriter.Write("   ");
                if (_hm < 10)
                {
        ...
        private void NauticalTwilightCalc()
        {
            ...
            // For Holy Days, we calculate SAM but don't write output
            _an = (259.18 - 1934.142 * _jt) * DR;
            _eo = (23.452294 - 0.0130125 * _jt - 0.00000164 * Math.Pow(_jt, 2) + 0.000000503 * Math.Pow(_jt, 3) + 0.00256 * Math.Cos(_an)) * DR;
            _a9 = 153.23 + 22518.7541 * _jt;
            _a9 = (_a9 - (Math.Floor(_a9 / 360)) * 360) * DR;
        ...
        private void PrintDayAndDate()
        {
            // Matching Windows PrintDayAndDate (lines 1114-1140)
            if (_ejwString == "Y")
            {
                _aa++;
            }
            ...
            if ((_aa <= JDNJLD + 0.5) || false) // Mode check removed for simplicity
            {
                _d = (a - Math.Floor(a)) * 7 + 1;
            }
            else
            {
                _d = (a - Math.Floor(a)) * 7;
            ...
            _outputWriter.Write(DayName[Convert.ToInt32(_d)]);
            PrintDate();
        }

        private void PrintDate()
        {
            // Matching Windows PrintDate (lines 1145-1176)
            double h = Math.Floor((_aa - 1867216.25) / 36524.25);
            h = _aa + 1 + h - Math.Floor(h / 4);
            double b = h + 1524;
            double c = Math.Floor((b - 122.1) / 365.25);
            double g = Math.Floor(365.25 * c);
            double e = Math.Floor((b - g) / 30.6001);
            double daye = b - g - Math.Floor(30.6001 * e) + _jf;
            ...
            string dayString = Math.Floor(daye).ToString(CultureInfo.InvariantCulture);
            dayString = dayString.TrimStart(' ', '-');
            while (dayString.Length < 2)
            {
                dayString = " " + dayString;
            ...
            _outputWriter.Write($", {dayString} {MonthName[(int)m]} {FormatYear(gyear)}");
            if (!_sunsetFlag)
            {
                _outputWriter.WriteLine("");
            }
        ...
        {
            // Match Windows Conversion.Str behavior
        ...
        private string FormatYear(double gyear)
        {
            // Match Windows StringYear function (lines 1033-1052)
            string intStringYear = Math.Abs(gyear).ToString(CultureInfo.InvariantCulture);
            // Remove leading space from Conversion.Str equivalent
            intStringYear = intStringYear.TrimStart(' ', '-');
            if (!_sunsetFlag)
            {
                if (gyear < 0)
                {
